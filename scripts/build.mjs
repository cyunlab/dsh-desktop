...
await Promise.all([
  build({ entryPoints: ['src/main/index.ts'], outfile: 'dist/main/index.js', bundle: true, platform: 'node', format: 'esm', external: ['electron', '@deepseek-ai/*', 'node-addon-require-builtin'], sourcemap: true, define, minifySyntax: true, plugins: [e2eObserver] }),
...
if (!e2e) {
  ...
  for (const forbidden of ['DSH_DESKTOP_TEST_HOST', 'DSH_DESKTOP_TEST_FAILURES', 'DSH_DESKTOP_TEST_USER_DATA', 'DSH_DESKTOP_TEST_EVENTS', 'Synthetic Host startup failure']) {
    if (main.includes(forbidden)) throw new Error(`production main bundle contains E2E hook: ${forbidden}`)
  }
}
