...
use lifecycle::{stop_process, LifecycleAction, LifecycleMachine, ProcessControl, SidecarEvent};
...
use windows_sys::Win32::System::Threading::{
    OpenProcess, CREATE_NEW_PROCESS_GROUP, PROCESS_QUERY_LIMITED_INFORMATION, PROCESS_SET_QUOTA,
    PROCESS_TERMINATE,
};
...
struct ProcessOwnership {
    ...
    job: Option<isize>,
    /// 用于无法附加 Job 时的 taskkill 兜底 PID。
    pid: u32,
}

#[cfg(windows)]
impl ProcessOwnership {
    /// 为新进程创建并附加带有 kill-on-close 语义的 Job Object。
    fn attach(pid: u32) -> Self {
        let job = unsafe { CreateJobObjectW(std::ptr::null(), std::ptr::null()) };
        if job.is_null() {
            return Self { job: None, pid };
        }
    ...
    /// 终止 Job Object 中的整个 sidecar 进程树，失败时交给 taskkill 兜底。
    ...
    /// 查询 Job Object 中是否仍有 sidecar 或其后代存活。
    fn tree_has_exited(&self) -> bool {
        ...
        !queried || accounting.ActiveProcesses == 0
    }

    /// 等待 Job Object 中的后代全部离开后再允许生命周期继续。
    fn wait_for_tree(&self) {
        while !self.tree_has_exited() {
...
impl ProcessControl for SidecarProcess {
    ...
    fn force_kill_tree(&self) -> Result<(), String> {
        let tree_result = self.ownership.force_kill_tree();
...
/// 移除错误文本中的绝对路径和换行，避免诊断复制泄露本机目录信息。
...
fn wait_for_web_listener(
    ...
    loop {
        if !is_current(state, generation) {
            return Err("startup attempt cancelled".into());
        }
        if TcpStream::connect_timeout(&address, Duration::from_millis(500)).is_ok() {
            return Ok(());
        }
        if !prolonged && started.elapsed() >= PROLONGED_STARTUP_AFTER {
            prolonged = true;
            transition(
                app,
                state,
...
fn spawn_sidecar(
    ...
    state: &RuntimeState,
    generation: u64,
) -> Result<(Arc<SidecarProcess>, mpsc::Receiver<SidecarMessage>), String> {
    let _gate = state
        .lifecycle_gate
        .lock()
        .map_err(|_| "lifecycle gate lock poisoned".to_string())?;
    if !is_current(state, generation) {
        return Err("startup attempt cancelled before spawning Host".into());
    }
    ...
    let mut child = command
        .spawn()
        .map_err(|error| format!("failed to spawn Node sidecar: {error}"))?;
    let stdin = match child.stdin.take() {
        Some(stdin) => stdin,
        None => {
            let _ = child.kill();
            let _ = child.wait();
            return Err("Node sidecar stdin unavailable".into());
        }
    };
    let stdout = match child.stdout.take() {
        ...
        None => {
            let _ = child.kill();
            let _ = child.wait();
            return Err("Node sidecar stdout unavailable".into());
        }
    };
    let process = Arc::new(SidecarProcess {
        ownership: ProcessOwnership::attach(child.id()),
    ...
    if !is_current(state, generation) {
        ...
        return Err("startup attempt cancelled after spawning Host".into());
    }
    if let Ok(mut current) = state.process.lock() {
        *current = Some(Arc::clone(&process));
    } else {
        stop_sidecar(&process);
        return Err("sidecar process lock poisoned".into());
    }
...
fn stop_sidecar(process: &Arc<SidecarProcess>) {
    let Ok(_stop_guard) = process.stop_lock.lock() else {
        return;
    };
    let _ = stop_process(process.as_ref(), SIDECAR_STOP_AFTER);
}
...
fn fail_attempt(app: &AppHandle, state: &RuntimeState, generation: u64, error: impl Into<String>) {
    ...
    let message = sanitize_diagnostic_text(&error.into());
    ...
    loop {
        ...
        if let Ok(message) = receiver.recv_timeout(Duration::from_millis(100)) {
            match message {
                SidecarMessage::Ready { origin } => {
                    if !is_allowed_host_origin(&origin) {
                        abort_attempt(
                            ...
                            generation,
                            &process,
                            "Host reported an invalid loopback origin",
                        );
                        return;
                    }
                    match machine.observe(SidecarEvent::Ready(origin.clone())) {
                        LifecycleAction::Ready(origin) => {
                            ...
                            if let Err(error) =
                                wait_for_web_listener(&app, &state, generation, &origin)
                            {
                                if is_current(&state, generation) {
                                    abort_attempt(&app, &state, generation, &process, error);
                                }
                                return;
                            }
                ...
                SidecarMessage::StartupFailed { error } => {
                    let event =
                        SidecarEvent::StartupFailed(format!("{}: {}", error.name, error.message));
                    if let LifecycleAction::Fail(message) = machine.observe(event) {
                        abort_attempt(&app, &state, generation, &process, message);
                    }
                    return;
                }
                SidecarMessage::StopFailed { error } => {
                    let event =
                        SidecarEvent::StartupFailed(format!("{}: {}", error.name, error.message));
                    if let LifecycleAction::Fail(message) = machine.observe(event) {
                        abort_attempt(&app, &state, generation, &process, message);
                ...
                SidecarMessage::Stopped => {
                    if let LifecycleAction::Fail(message) = machine.observe(SidecarEvent::Stopped) {
                        abort_attempt(&app, &state, generation, &process, message);
                    }
                    return;
                }
            ...
            if let Ok(Some(status)) = child.try_wait() {
                if is_current(&state, generation) {
                    let event = if matches!(
                        machine.observe(SidecarEvent::Exited),
                        LifecycleAction::Fail(_)
                    ) {
                        Some(format!("Host exited unexpectedly ({status})"))
                    } else {
                        None
                    };
...
fn build_startup_diagnostics(state: &RuntimeState, snapshot: &LifecycleSnapshot) -> String {
    ...
    let diagnostics = StartupDiagnostics {
        ...
        node_version: std::env::var("DSH_NODE_VERSION").unwrap_or_else(|_| "24.19.0".into()),
        platform: std::env::consts::OS.into(),
        arch: std::env::consts::ARCH.into(),
        lifecycle_state: serde_json::to_string(&snapshot.state)
            .unwrap_or_else(|_| "unknown".into()),
        lifecycle_message: snapshot.message.clone(),
...
mod tests {
    use super::{
        bundled_node_relative_path, decide_navigation, is_allowed_host_origin,
        parse_sidecar_message, sanitize_diagnostic_text, LifecycleState, NavigationDecision,
        SidecarMessage,
    };
    /// 验证生命周期 ready 消息可解析。
    #[test]
    ...
    fn bundled_node_layout_matches_platform_archive() {
        ...
        if cfg!(windows) {
            ...
            assert_eq!(relative.to_string_lossy(), "bin/node");
        }
    }

    /// 验证复制诊断会把 Windows、Unix 和 file URL 路径替换为占位符。
    #[test]
    fn diagnostics_redact_absolute_paths() {
        let redacted = sanitize_diagnostic_text(
            r#"failed at C:\Users\alice\app\node.exe /var/lib/dsh file:///private/key"#,
        );
