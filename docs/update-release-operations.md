...
- a real Windows window close, Linux X11 window close, or macOS normal application Quit triggers confirmed Host cleanup and installation;
- normal close/quit does not automatically relaunch; the harness separately launches the same installation location and observes the candidate version plus fixed Host origin ready;
...
The long-lived repository driver installs the exact OSS-selected v2.1.5 package, verifies both baseline and candidate minisign signatures with the embedded public key, inspects runtime closure from both installed trees, and binds configuration identity to the real process. It then observes staging, performs the platform-native close/quit, verifies Host shutdown and in-place replacement, proves there was no automatic relaunch, and manually starts the candidate. Fixture adapters remain `local-fixture` and cannot satisfy `--require-real-native`.
