...
The first runnable milestone uses the upstream Web profile composition: `@deepseek-ai/dsh-base` followed by `@deepseek-ai/dsh-web-app`. These are aggregate bundles whose transitive dependencies provide and configure the full standard Web Host and Client plugin roster. Desktop also declares the small set of launcher/runtime packages required to boot that composition inside the official Node sidecar; it does not copy the bundle patches or curate a reduced plugin list.
