...
For the first runnable milestone, the Harness Host boots inside the Electron main process and serves the Web Client over a random loopback port. The BrowserWindow first displays a packaged local startup page. After the Host reports that its loopback listener is ready, the window navigates to the Web Client; startup failure leaves the local page available to show the error and offer retry and log-copy actions. The Host startup remains behind a launcher interface so crash isolation in a utility or child process can be evaluated after the end-to-end desktop path works.
...
The Web Client remains the owner of the durable multi-Workspace experience. Desktop does not add a second project picker before Host startup. Before boot, it assigns the Host a deterministic empty fallback working directory under Desktop-owned application data so a Session created without a Workspace or explicit cwd never inherits an installer, shortcut, or terminal directory.
...
Desktop requests port `0`, allowing the operating system to allocate the listener atomically. Readiness requires the Harness boot/configuration tree to settle, the assigned port to be read from the `webServer` service, and a bounded loopback HTTP probe to succeed before navigation. Log output such as `dsh web: ...` is diagnostic only and is not a lifecycle protocol.
