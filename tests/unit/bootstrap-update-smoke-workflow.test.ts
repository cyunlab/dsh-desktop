...
describe('bootstrap update smoke reusable workflow contract', () => {
  ...
  it('runs the exact four-target production matrix against one immutable candidate', () => {
    ...
    expect(workflow).toContain('BOOTSTRAP_TAG: ${{ vars.UPDATER_BOOTSTRAP_TAG }}')
    expect(workflow).toContain('BOOTSTRAP_VERSION: ${{ vars.UPDATER_BOOTSTRAP_VERSION }}')
    expect(workflow).toContain('BOOTSTRAP_COMMIT: ${{ vars.UPDATER_BOOTSTRAP_COMMIT }}')
    expect(workflow).toContain('RECOVERY_TAG: ${{ vars.UPDATER_RECOVERY_TAG }}')
    expect(workflow).toContain('broken-updater-recovery)')
    ...
    expect(workflow).toContain('BOOTSTRAP_SIGNING_CONFIGURED: ${{ vars.UPDATER_BOOTSTRAP_MACOS_SIGNING_CONFIGURED }}')
    expect(workflow).toContain('RECOVERY_SIGNING_CONFIGURED: ${{ vars.UPDATER_RECOVERY_MACOS_SIGNING_CONFIGURED }}')
    expect(workflow).toContain('test "$SIGNING_CONFIGURED" = true')
  })
