...
describe('bootstrap update smoke producer public seam', () => {
  ...
  it('writes checksummed bootstrap-local-fixture evidence from a complete test adapter', async () => {
    ...
    const result = await runBootstrapUpdateSmoke(options, { DSH_BOOTSTRAP_UPDATE_SMOKE_DRIVER: '/test/adapter', DSH_BOOTSTRAP_MACOS_APPLICATIONS_STAGING: 'true', PATH: '/trusted/bin', HOME: '/home/runner', TMPDIR: '/tmp', ComSpec: 'C:\\Windows\\System32\\cmd.exe', GH_TOKEN: 'secret' }, {
    ...
    expect(invocation.environment).toEqual({ PATH: '/trusted/bin', HOME: '/home/runner', TMPDIR: '/tmp', ComSpec: 'C:\\Windows\\System32\\cmd.exe', DSH_BOOTSTRAP_MACOS_APPLICATIONS_STAGING: 'true', DSH_BOOTSTRAP_UPDATE_SMOKE_OUTPUT: 'json' })
  })
})
